...
public:
	...
	{}
	explicit operator slib::vec3() const
	{
		return{ float( GetR() ),float( GetG() ), float( GetB() ) };
	}
	constexpr unsigned char Saturate(float value) const
	{
	return static_cast<unsigned char>(value < 0.0f ? 0u : (value > 1.0f ? 255u : value));
	}
	explicit Color(const slib::vec3& vec)
	:
	dword(
		(0xff << 24u) |
		(Saturate(vec.x) << 16u) |
